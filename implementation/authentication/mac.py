...
if __name__ == '__main__':
    key = 'axxgjjpvnon&d'
    nonce = 'b' * 128  # (X = 64 for sha-1 or X = 128 for hmac)
    ...
    #assert sha_1_mac == generate_mac_sha1(content.encode(), key, nonce.encode())
    assert hmac_mac == generate_mac_hmac(content.encode(), key, nonce.encode())
